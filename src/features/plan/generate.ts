...
function buildUserPrompt(input: PlanInput): string {
  ...
  lines.push(
    input.endDate
      ? `Plan length: reach the goal by ${input.endDate} — about ${input.durationWeeks} week(s) from today.`
      : `Plan length: ${input.durationWeeks} week(s).`,
  );
...
export function buildPlan(gen: GeneratedPlan, input: PlanInput, liability: LiabilityAck): Plan {
  ...
  // Honor the user's exact target date when given; else derive from weeks.
  const endDate =
    input.endDate && input.endDate > startDate
      ? input.endDate
      : shiftDate(startDate, input.durationWeeks * 7 - 1);
